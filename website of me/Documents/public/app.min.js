...
const HOME_CONSOLE_LIMIT = 12;
const HOME_BLOG_PREVIEW_LIMIT = 3;
const HOME_AI_TOOL_PREVIEW_LIMIT = 6;
...
async function loadPublicContent(pageKeys = ["home", "games", "cart"]) {
  const data = await api("/api/content/public");
  const cachedPageContent = {
    ...(state.content?.pageContent || {})
  };
  const contentKeys = Array.isArray(pageKeys) ? pageKeys.filter(Boolean) : [];

  if (contentKeys.length) {
    const pageEntries = await Promise.all(contentKeys.map(async (pageKey) => [pageKey, await loadPageContent(pageKey)]));
    pageEntries.forEach(([pageKey, content]) => {
      if (content) cachedPageContent[pageKey] = content;
    });
  }
...
async function loadPageContent(pageKey, options = {}) {
  const { force = false } = options;
  const cached = state.content?.pageContent?.[pageKey];
...
async function loadAdminContent() {
  ...
  const [homeContent, gamesContent, cartContent] = await Promise.all([
    loadPageContent("home", { force: true }),
    loadPageContent("games", { force: true }),
    loadPageContent("cart", { force: true })
  ]);
  state.content = {
    sections: data.sections || [],
    pages: data.pages || [],
...
async function loadConsoleGames(options = {}) {
  const {
    limit = 0,
    queryByPlatform = {}
  } = options;
  const [ps4, ps5] = await Promise.all(["PS4", "PS5"].map(async (platform) => {
    const params = new URLSearchParams();
    params.set("platform", platform);
    const query = String(queryByPlatform?.[platform] || "").trim();
    if (query) params.set("q", query);
    if (limit) params.set("limit", String(limit));
    return api(`/console-games?${params.toString()}`);
  }));
  state.consoleGames = {
    PS4: ps4.items || [],
    PS5: ps5.items || []
...
async function refreshCart(options = {}) {
  ...
  try {
    ...
    if (!silent) {
      console.warn("Cart sync failed. Using cached cart data.", error);
    }
  }
...
async function loadBlogs(options = {}) {
  ...
  const endpoint = params.toString() ? `/blogs?${params.toString()}` : "/blogs";

  state.blogs = (await api(endpoint)).sort((left, right) => {
...
async function loadAiTools(options = {}) {
  const {
    limit = 0,
    featured = false,
    query = "",
  ...
  if (query) params.set("q", query);
  if (category) params.set("category", category);
  const endpoint = params.toString() ? `/api/ai-tools?${params.toString()}` : "/api/ai-tools";
...
async function initHome() {
  ...
  await Promise.all([
    loadPublicContent(["home"]),
    loadConsoleGames({ limit: HOME_CONSOLE_LIMIT }),
    loadGames({ reset: true, query: initialQuery, category: initialCategory })
  ]);

  applyManagedSections();
  const homePageContent = state.content?.pageContent?.home || await loadPageContent("home");
  ...
  scheduleIdleTask(async () => {
    try {
      ...
      renderWishlistPreview();
      await Promise.all([
        loadBlogs({ limit: HOME_BLOG_PREVIEW_LIMIT, preview: true }),
        loadAiTools({ featured: true, limit: HOME_AI_TOOL_PREVIEW_LIMIT }),
        loadHomeShowcase()
      ]);
...
async function initGamesCatalog() {
  const gamesPageContent = state.content?.pageContent?.games || await loadPageContent("games");
...
async function initConsoleCatalogPage(platform, pageKey) {
  ...
  await loadBlogs({ limit: RELATED_BLOG_LIMIT, preview: true });
...
async function initCart() {
  const cartPageContent = state.content?.pageContent?.cart || await loadPageContent("cart");
  ...
  if (cartPageContent?.buttonLabel && qs("goCheckoutBtn")) qs("goCheckoutBtn").textContent = cartPageContent.buttonLabel;
    if (!state.cartLoaded) await refreshCart({ silent: true });
    renderCartPage();
  }
